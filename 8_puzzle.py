...
def astar_manhattan_distance(initial_state):
    # Initialize the open and closed sets
    open_set = []
    closed_set = set()
    heapq.heappush(open_set, (manhattan_distance(initial_state), initial_state, []))
    max_heap_size = 1
    nodes_expanded = 0
    ...
    while open_set:
        ...
        if blank_index > 2:
            ...
            new_state[blank_index], new_state[blank_index - 3] = new_state[blank_index - 3], new_state[blank_index]
            if tuple(new_state) not in closed_set:
                successors.append((manhattan_distance(new_state) + len(path) + 1, new_state, path + ['Up']))
        if blank_index < 6:
            new_state = state[:]
            new_state[blank_index], new_state[blank_index + 3] = new_state[blank_index + 3], new_state[blank_index]
            if tuple(new_state) not in closed_set:
                successors.append((manhattan_distance(new_state) + len(path) + 1, new_state, path + ['Down']))
        if blank_index % 3 > 0:
            new_state = state[:]
            new_state[blank_index], new_state[blank_index - 1] = new_state[blank_index - 1], new_state[blank_index]
            if tuple(new_state) not in closed_set:
                successors.append((manhattan_distance(new_state) + len(path) + 1, new_state, path + ['Left']))
        if blank_index % 3 < 2:
            new_state = state[:]
            new_state[blank_index], new_state[blank_index + 1] = new_state[blank_index + 1], new_state[blank_index]
            if tuple(new_state) not in closed_set:
                successors.append((manhattan_distance(new_state) + len(path) + 1, new_state, path + ['Right']))
...
for i, test_case in enumerate(test_cases):
    solution, depth, max_heap_size, nodes_expanded = astar_manhattan_distance(test_case)
    print(f"Test Case {i+1}:")
    print("Solution:", solution)
    print("Depth of solution:", depth)
    print("Maximum heap size:", max_heap_size)
